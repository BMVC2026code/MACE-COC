"""LLaVA-Med generator for medical VQA."""
...
PROMPT = """[INST] <image>
You are answering a radiology medical visual question.
...
def main():
    ...
    gen = LlavaMedGenerator(device=args.device)

    for idx, item in enumerate(tqdm(rows, desc=f"LLaVA-Med {args.dataset}")):
        if idx < len(results):
            continue
